...
const RULE_VALUE_HINTS = {
    ...
    Account_Segment: {
        label: 'Account Segment Expression',
        placeholder: 'Format: FieldApiName:Value (e.g. Industry:Technology)'
    },
    Territory: { label: 'Territories', placeholder: 'Filter territories…' }
};

const SEGMENT_COLORS = ['#0176d3', '#6366f1', '#22c55e', '#f59e0b', '#ec4899', '#14b8a6', '#f97316', '#8b5cf6'];

/** Rule types that use the lookup input instead of plain text */
const LOOKUP_RULE_TYPES = new Set(['User', 'Profile', 'Permission_Set', 'Account', 'Territory']);
...
const LOOKUP_HELP = {
    ...
    Permission_Set:
        'Select one or more permission sets. The rule matches if the evaluating user has any of these assigned.',
    Account: 'Search for one or more accounts. The rule matches if the context account ID is in this list.',
    Territory:
        'Select one or more territories. The rule matches if the evaluating user is assigned to any selected territory.'
};
...
export default class FeatureFlagRuleBuilder extends LightningElement {
    ...
    _enrichRule(r, idx) {
        ...
        const valueSummary = rawValue.length > 60 ? rawValue.substring(0, 57) + '…' : rawValue;
    ...
    async handleSaveVariant(event) {
        ...
        try {
            ...
            if (isNew) {
                this.localVariants = this.localVariants.map((v) => (v.id === id ? { ...v, id: saved.Id } : v));
            }
